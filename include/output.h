...
/**
 * Small robustness helpers for output code.
 *
 * These are intentionally lightweight and header-only so they can be
 * used inside output.c without changing the public API.
 */
#ifndef _OUTPUT_ABS_
#define _OUTPUT_ABS_(x) (((x) < 0.) ? -(x) : (x))
#endif

#ifndef _OUTPUT_ISFINITE_
#define _OUTPUT_ISFINITE_(x) isfinite((double)(x))
#endif

#ifndef _OUTPUT_SAFE_MIN_POSITIVE_
#define _OUTPUT_SAFE_MIN_POSITIVE_ DBL_MIN
#endif

#ifndef _OUTPUT_ROOT_BUFFER_GUARD_
#define _OUTPUT_ROOT_BUFFER_GUARD_ 32
#endif
