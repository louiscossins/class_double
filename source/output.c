...
static int output_check_finite(double x) {
  return isfinite(x);
}

static double output_safe_exp(double x) {
  if (x > log(DBL_MAX)) return DBL_MAX;
  if (x < log(DBL_MIN)) return 0.0;
  return exp(x);
}

static double output_safe_sqrt_nonneg(double x) {
  if (x <= 0.0) return 0.0;
  return sqrt(x);
}
...
int output_total_cl_at_l(
  ...
  class_test(psp == NULL || ple == NULL || pop == NULL || cl == NULL,
             pop->error_message,
             "null pointer passed to output_total_cl_at_l");

  class_test(l < 2,
             pop->error_message,
             "requested multipole l=%d, but output_total_cl_at_l expects l>=2", l);
...
int output_init(
  ...
  class_test(pba == NULL || pth == NULL || ppt == NULL || ppm == NULL || ptr == NULL || psp == NULL || pnl == NULL || ple == NULL || pop == NULL,
             pop->error_message,
             "null pointer passed to output_init");
...
int output_pk(
  ...
  class_test(pba == NULL || ppt == NULL || pnl == NULL || pop == NULL,
             pop->error_message,
             "null pointer passed to output_pk");

  class_test(pba->h == 0. || output_check_finite(pba->h) == 0,
             pop->error_message,
             "invalid value h=%e in output_pk", pba->h);

  class_test(pnl->k_size <= 0,
             pop->error_message,
             "non-positive k_size=%d in output_pk", pnl->k_size);
  ...
  for (index_pk=0; index_pk<pnl->pk_size; index_pk++) {
    ...
    for (index_z = 0; index_z < pop->z_pk_num; index_z++) {
      ...
      class_test(output_check_finite(pop->z_pk[index_z]) == 0,
                 pop->error_message,
                 "non-finite requested redshift z_pk[%d]=%e in output_pk", index_z, pop->z_pk[index_z]);
      ...
      for (index_k=0; index_k<pnl->k_size; index_k++) {
        ...
        class_test(output_check_finite(pnl->ln_k[index_k]) == 0 || output_check_finite(ln_pk[index_k]) == 0,
                   pop->error_message,
                   "non-finite P(k) entry at index_k=%d in output_pk", index_k);

        class_call(output_one_line_of_pk(out_pk,
                                         output_safe_exp(pnl->ln_k[index_k])/pba->h,
                                         output_safe_exp(ln_pk[index_k])*pow(pba->h,3)
                                         ),
                   pop->error_message,
                   pop->error_message);
        ...
        if (do_ic == _TRUE_) {
          ...
          for (index_ic1_ic2 = 0; index_ic1_ic2 < pnl->ic_ic_size; index_ic1_ic2++) {
            ...
            if (pnl->is_non_zero[index_ic1_ic2] == _TRUE_) {
              ...
              class_test(output_check_finite(ln_pk_ic[index_k * pnl->ic_ic_size + index_ic1_ic2]) == 0,
                         pop->error_message,
                         "non-finite ic-resolved P(k) entry at index_k=%d index_ic1_ic2=%d in output_pk",
                         index_k,
                         index_ic1_ic2);

              class_call(output_one_line_of_pk(out_pk_ic[index_ic1_ic2],
                                               output_safe_exp(pnl->ln_k[index_k])/pba->h,
                                               output_safe_exp(ln_pk_ic[index_k * pnl->ic_ic_size + index_ic1_ic2])*pow(pba->h,3)),
                         pop->error_message,
                         pop->error_message);
            }
          }
...
int output_tk(
  ...
  class_test(pba == NULL || ppt == NULL || pop == NULL,
             pop->error_message,
             "null pointer passed to output_tk");

  class_test(pba->h == 0. || output_check_finite(pba->h) == 0,
             pop->error_message,
             "invalid value h=%e in output_tk", pba->h);
...
int output_background(
  ...
  class_test(pba == NULL || pop == NULL,
             pop->error_message,
             "null pointer passed to output_background");
...
int output_print_data(FILE *out,
  ...
  class_test(out == NULL || dataptr == NULL,
             out != NULL ? "output_print_data received invalid input" : "output_print_data received invalid input",
             "null pointer passed to output_print_data");

  class_test(size_dataptr < 0,
             out != NULL ? "output_print_data received invalid size" : "output_print_data received invalid size",
             "negative size_dataptr=%d in output_print_data", size_dataptr);
  ...
  class_test(number_of_titles > 0 && (size_dataptr % number_of_titles) != 0,
             "output_print_data inconsistent table size",
             "size_dataptr=%d is not divisible by number_of_titles=%d in output_print_data",
             size_dataptr,
             number_of_titles);
  if (number_of_titles>0){
    for (index_tau=0; index_tau<size_dataptr/number_of_titles; index_tau++){
      ...
      for (index_title=0; index_title<number_of_titles; index_title++){
        class_test(output_check_finite(dataptr[index_tau*number_of_titles+index_title]) == 0,
                   "output_print_data encountered non-finite value",
                   "non-finite table entry at row=%d col=%d in output_print_data",
                   index_tau,
                   index_title);
...
int output_open_cl_file(
  ...
  class_test(psp == NULL || pop == NULL || clfile == NULL,
             pop->error_message,
             "null pointer passed to output_open_cl_file");

  class_test(filename[0] == '\0',
             pop->error_message,
             "empty filename passed to output_open_cl_file");

  class_test(lmax < 2,
             pop->error_message,
             "invalid lmax=%d in output_open_cl_file", lmax);
...
int output_one_line_of_cl(
  ...
  class_test(pba == NULL || psp == NULL || pop == NULL || clfile == NULL || cl == NULL,
             pop->error_message,
             "null pointer passed to output_one_line_of_cl");

  class_test(output_check_finite(l) == 0 || l < 2.,
             pop->error_message,
             "invalid multipole l=%e in output_one_line_of_cl", l);

  class_test(ct_size < 0,
             pop->error_message,
             "negative ct_size=%d in output_one_line_of_cl", ct_size);
  ...
  if (pop->output_format == class_format) {
    ...
    for (index_ct=0; index_ct < ct_size; index_ct++) {
      class_test(output_check_finite(cl[index_ct]) == 0,
                 pop->error_message,
                 "non-finite C_l entry at index_ct=%d in output_one_line_of_cl", index_ct);
  ...
  if (pop->output_format == camb_format) {
    if (psp->has_tt == _TRUE_) class_test(output_check_finite(cl[psp->index_ct_tt]) == 0, pop->error_message, "non-finite TT C_l in output_one_line_of_cl");
    if (psp->has_ee == _TRUE_) class_test(output_check_finite(cl[psp->index_ct_ee]) == 0, pop->error_message, "non-finite EE C_l in output_one_line_of_cl");
    if (psp->has_bb == _TRUE_) class_test(output_check_finite(cl[psp->index_ct_bb]) == 0, pop->error_message, "non-finite BB C_l in output_one_line_of_cl");
    if (psp->has_te == _TRUE_) class_test(output_check_finite(cl[psp->index_ct_te]) == 0, pop->error_message, "non-finite TE C_l in output_one_line_of_cl");
    if (psp->has_pp == _TRUE_) class_test(output_check_finite(cl[psp->index_ct_pp]) == 0, pop->error_message, "non-finite PP C_l in output_one_line_of_cl");
    if (psp->has_tp == _TRUE_) class_test(output_check_finite(cl[psp->index_ct_tp]) == 0, pop->error_message, "non-finite TP C_l in output_one_line_of_cl");
    if (psp->has_ep == _TRUE_) class_test(output_check_finite(cl[psp->index_ct_ep]) == 0, pop->error_message, "non-finite EP C_l in output_one_line_of_cl");
    ...
    class_fprintf_double(clfile, output_safe_sqrt_nonneg(l*(l+1))*factor*pba->T_cmb*1.e6*cl[psp->index_ct_tp], psp->has_tp);
    class_fprintf_double(clfile, output_safe_sqrt_nonneg(l*(l+1))*factor*pba->T_cmb*1.e6*cl[psp->index_ct_ep], psp->has_ep);
    index_ct_rest = 0;
...
int output_open_pk_file(
  ...
  class_test(pba == NULL || pnl == NULL || pop == NULL || pkfile == NULL,
             pop->error_message,
             "null pointer passed to output_open_pk_file");

  class_test(filename[0] == '\0',
             pop->error_message,
             "empty filename passed to output_open_pk_file");

  class_test(pnl->k_size <= 0,
             pop->error_message,
             "non-positive k_size=%d in output_open_pk_file", pnl->k_size);

  class_test(pba->h == 0. || output_check_finite(pba->h) == 0,
             pop->error_message,
             "invalid value h=%e in output_open_pk_file", pba->h);
  ...
  if (pop->write_header == _TRUE_) {
    ...
    fprintf(*pkfile,"# for k=%g to %g h/Mpc,\n",
            output_safe_exp(pnl->ln_k[0])/pba->h,
            output_safe_exp(pnl->ln_k[pnl->k_size-1])/pba->h);
...
int output_one_line_of_pk(
  ...
  if (output_check_finite(one_k) == 0)
    one_k = 0.;
  if (output_check_finite(one_pk) == 0)
    one_pk = 0.;
