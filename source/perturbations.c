...
/* Robustness helpers */
#ifndef _PERTURB_EPS_
#define _PERTURB_EPS_ 1e-300
#endif

static int perturb_is_finite(double x) {
  return isfinite(x);
}

static double perturb_safe_log_tau(double tau) {
  if ((tau <= 0.0) || (!isfinite(tau))) return log(_PERTURB_EPS_);
  return log(tau);
}

static double perturb_safe_k2(double k) {
  double k2 = k*k;
  if ((!isfinite(k2)) || (fabs(k2) < _PERTURB_EPS_)) return _PERTURB_EPS_;
  return k2;
}
...
int perturb_sources_at_tau(
  ...
  class_test(psource == NULL,
             ppt->error_message,
             "psource pointer is NULL");

  class_test(index_md < 0 || index_md >= ppt->md_size,
             ppt->error_message,
             "index_md=%d out of range [0,%d)",index_md,ppt->md_size);

  class_test(index_ic < 0 || index_ic >= ppt->ic_size[index_md],
             ppt->error_message,
             "index_ic=%d out of range [0,%d) for mode %d",index_ic,ppt->ic_size[index_md],index_md);

  class_test(index_tp < 0 || index_tp >= ppt->tp_size[index_md],
             ppt->error_message,
             "index_tp=%d out of range [0,%d) for mode %d",index_tp,ppt->tp_size[index_md],index_md);

  class_test((tau <= 0.0) || (perturb_is_finite(tau) == 0),
             ppt->error_message,
             "tau=%e must be finite and strictly positive",tau);

  logtau = perturb_safe_log_tau(tau);
...
int perturb_output_data(
  ...
  class_test(pba == NULL,
             ppt->error_message,
             "background structure pointer is NULL");

  class_test(data == NULL,
             ppt->error_message,
             "data pointer is NULL");

  class_test((perturb_is_finite(z) == 0) || (z < 0.0),
             ppt->error_message,
             "requested redshift z=%e must be finite and non-negative",z);

  class_test((perturb_is_finite(pba->h) == 0) || (fabs(pba->h) < _PERTURB_EPS_),
             ppt->error_message,
             "background h=%e must be finite and non-zero",pba->h);
  ...
  /* if 0 <= z_pk <= z_max_pk, interpolation needed, */
    ...
    class_test((perturb_is_finite(tau) == 0) || (tau <= 0.0),
               ppt->error_message,
               "background_tau_of_z returned invalid tau=%e for z=%e",tau,z);

    class_test(perturb_safe_log_tau(tau) < ppt->ln_tau[0],
               ppt->error_message,
  ...
  for (index_ic = 0; index_ic < ppt->ic_size[index_md]; index_ic++) {
    ...
    for (index_k=0; index_k<ppt->k_size[index_md]; index_k++) {
      ...
      class_test(perturb_is_finite(k) == 0,
                 ppt->error_message,
                 "non-finite k encountered at index_k=%d",index_k);
      k2 = perturb_safe_k2(k);
      k_over_h = k/pba->h;
...
int perturb_init(
  ...
  if ((ppr == NULL) || (pba == NULL) || (pth == NULL) || (ppt == NULL)) {
    return _FAILURE_;
  }

  class_test((perturb_is_finite(pba->h) == 0) || (pba->h <= 0.0),
             ppt->error_message,
             "invalid background h=%e",pba->h);
...
int perturb_approximations(
  ...
  tau_k = 1./sqrt(perturb_safe_k2(k));
...
int perturb_timescale(
  ...
  tau_k = 1./sqrt(perturb_safe_k2(pppaw->k));
...
if (ppt->has_source_theta_scf == _TRUE_) {
  double rho_plus_p_scf;
  rho_plus_p_theta_scf = (1./3.)*(-Zbg*((k*k*y[ppw->pv->index_pt_phi_scf]/a_rel)+Fz*y[ppw->pv->index_pt_theta_cdm]));
  rho_plus_p_scf = pvecback[pba->index_bg_rho_scf] + pvecback[pba->index_bg_p_scf];

  if (fabs(rho_plus_p_scf) < 1e-40)
    _set_source_(ppt->index_tp_theta_scf) = 0.0;
  else
    _set_source_(ppt->index_tp_theta_scf) = rho_plus_p_theta_scf / rho_plus_p_scf;
}
    /* theta_dr */
...

{
  ...
  if (fabs(rho_plus_p_scf) < 1e-40)
    ...
    theta_scf = rho_plus_p_theta_scf / rho_plus_p_scf;
}
    }
