...
/* Numerical safety helpers for modified scalar-field background code */
static double background_safe_exp(double x) {
  /* Stay well inside IEEE double overflow/underflow limits */
  if (x > 700.0) x = 700.0;
  if (x < -700.0) x = -700.0;
  return exp(x);
}

static int background_is_finite(double x) {
  return isfinite(x);
}
...
int background_functions(
  ...
  if (pba->has_scf == _TRUE_) {
    ...
    class_test(background_is_finite(phi) == _FALSE_,
               pba->error_message,
               "non-finite scalar field value phi=%e",
               phi);

    class_test(background_is_finite(phi_prime) == _FALSE_,
               pba->error_message,
               "non-finite scalar field derivative phi_prime=%e",
               phi_prime);
    ...
    pvecback[pba->index_bg_lambda_eff_scf] = lambda_eff_scf(pba,phi);

    class_test(background_is_finite(pvecback[pba->index_bg_V_scf]) == _FALSE_,
               pba->error_message,
               "non-finite V_scf(phi=%e)",
               phi);

    class_test(background_is_finite(pvecback[pba->index_bg_dV_scf]) == _FALSE_,
               pba->error_message,
               "non-finite dV_scf(phi=%e)",
               phi);

    class_test(background_is_finite(pvecback[pba->index_bg_ddV_scf]) == _FALSE_,
               pba->error_message,
               "non-finite ddV_scf(phi=%e)",
               phi);

    class_test(background_is_finite(pvecback[pba->index_bg_lambda_eff_scf]) == _FALSE_,
               pba->error_message,
               "non-finite lambda_eff_scf(phi=%e)",
               phi);
    ...
    pvecback[pba->index_bg_rho_scf] = (0.5*Zbg*Zbg + F - Zbg*Fz + pvecback[pba->index_bg_V_scf])/3.;
    pvecback[pba->index_bg_p_scf]   = (0.5*Zbg*Zbg - F - pvecback[pba->index_bg_V_scf])/3.;

    class_test(background_is_finite(pvecback[pba->index_bg_rho_scf]) == _FALSE_,
               pba->error_message,
               "non-finite rho_scf(phi=%e,phi_prime=%e)",
               phi,
               phi_prime);

    class_test(background_is_finite(pvecback[pba->index_bg_p_scf]) == _FALSE_,
               pba->error_message,
               "non-finite p_scf(phi=%e,phi_prime=%e)",
               phi,
               phi_prime);

    if (fabs(pvecback[pba->index_bg_rho_scf]) > 1e-300)
      w_phi_scf = pvecback[pba->index_bg_p_scf] / pvecback[pba->index_bg_rho_scf];
    else
      w_phi_scf = -1.0;

    class_test(background_is_finite(w_phi_scf) == _FALSE_,
               pba->error_message,
               "non-finite w_phi_scf(phi=%e,phi_prime=%e)",
               phi,
               phi_prime);

    pvecback[pba->index_bg_w_phi_scf] = w_phi_scf;
  ...
  h2_arg = rho_tot-pba->K/a/a;
  if ((h2_arg < 0.) && (fabs(h2_arg) < 1e-14*(fabs(rho_tot)+fabs(pba->K/a/a)+1.))) {
    h2_arg = 0.;
  }
  class_test(h2_arg < 0.,
             pba->error_message,
             "negative argument in Friedmann equation: rho_tot-K/a^2 = %e",
             h2_arg);
  pvecback[pba->index_bg_H] = sqrt(h2_arg);
  ...
  if (pba->has_scf == _TRUE_){
    ...
    class_test(background_is_finite(pvecback[pba->index_bg_p_prime_scf]) == _FALSE_,
               pba->error_message,
               "non-finite p_prime_scf(phi=%e,phi_prime=%e)",
  ...
  /* Total effective equation of state */
  if (fabs(rho_tot) > 1e-300)
    pvecback[pba->index_bg_w_eff] = p_tot / rho_tot;
  else
    pvecback[pba->index_bg_w_eff] = -1.0;
...
int background_indices(
  ...
  class_define_index(pba->index_bg_lambda_eff_scf,pba->has_scf,index_bg,1);
  ...
  class_define_index(pba->index_bg_w_phi_scf,pba->has_scf,index_bg,1);
  ...
  /* - index for total effective equation of state */
  class_define_index(pba->index_bg_w_eff,_TRUE_,index_bg,1);
 ...
 *
 * For the double-exponential potential
 *
 *   V(phi) = V1 [ exp(-lambda1*phi) + A exp(-lambda2*phi) ]
 *
 * the old radiation-attractor initial conditions used for a single
 * exponential are not valid in general.
 *
 * Therefore, we always use explicit user-provided initial conditions:
 *
 *   phi_ini_scf
 *   phi_prime_ini_scf
 *
 * If attractor_ic_scf was set to TRUE in the input, we ignore it and
 * fall back to manual ICs, with a warning.
 */
if (pba->has_scf == _TRUE_) {

  if (pba->attractor_ic_scf == _TRUE_) {
    if (pba->background_verbose > 0) {
      printf("Warning: attractor_ic_scf requested, but disabled for double-exponential potential.\n");
      printf("         Using explicit initial conditions phi_ini_scf and phi_prime_ini_scf instead.\n");
    }
  }

  /* Always use explicit scalar-field initial conditions */
  ...
  class_test(!isfinite(pvecback_integration[pba->index_bi_phi_scf]) ||
             ...
             pba->error_message,
             "initial phi = %e phi_prime = %e -> check scalar-field initial conditions",
             pvecback_integration[pba->index_bi_phi_scf],
             pvecback_integration[pba->index_bi_phi_prime_scf]);
}
...
int background_output_titles(struct background * pba,
  ...
  class_store_columntitle(titles,"(.)w_phi_scf",pba->has_scf);
  ...
  class_store_columntitle(titles,"V''_scf",pba->has_scf);
  class_store_columntitle(titles,"lambda_eff_scf",pba->has_scf);

  class_store_columntitle(titles,"(.)rho_tot",_TRUE_);
  class_store_columntitle(titles,"(.)p_tot",_TRUE_);
  class_store_columntitle(titles,"(.)w_eff",_TRUE_);
...
int background_output_data(
  ...
  for (index_tau=0; index_tau<pba->bt_size; index_tau++){
    ...
    class_store_double(dataptr,pvecback[pba->index_bg_w_phi_scf],pba->has_scf,storeidx);
    ...
    class_store_double(dataptr,pvecback[pba->index_bg_lambda_eff_scf],pba->has_scf,storeidx);
    ...
    class_store_double(dataptr,pvecback[pba->index_bg_w_eff],_TRUE_,storeidx);
...
int background_derivs(
  ...
  if (pba->has_scf == _TRUE_){
    ...
    /** - Scalar field equation with explicit protection against the singular case 1-Fzz = 0 */
    ...
    class_test(fabs(denom_kg) < 1e-14,
               error_message,
               "scalar-field evolution is singular because 1-Fzz = %e (scf_veta=%e)",
               denom_kg,
               pba->scf_veta);

    dy[pba->index_bi_phi_prime_scf] = y[pba->index_bi_a]*pvecback[pba->index_bg_H]*y[pba->index_bi_phi_prime_scf]
      -3.*y[pba->index_bi_a]*y[pba->index_bi_a]*pvecback[pba->index_bg_H]*(Fz-Zbg)/denom_kg
      -y[pba->index_bi_a]*y[pba->index_bi_a]*dV_scf(pba,y[pba->index_bi_phi_scf])/denom_kg;

    class_test(background_is_finite(dy[pba->index_bi_phi_prime_scf]) == _FALSE_,
               error_message,
               "non-finite phi_prime' encountered during background integration");
 ...
 *
 * where
 *   scf_parameters[0] = lambda1
 *   scf_parameters[1] = lambda2
 *   scf_parameters[2] = A = V2/V1
 *   scf_parameters[3] = V1   (optional, defaults to 1)
 *
 * The effective slope is
 *
 *   lambda_eff(phi) =
 *   (lambda1*e^{-lambda1 phi} + A*lambda2*e^{-lambda2 phi}) /
 *   (e^{-lambda1 phi} + A*e^{-lambda2 phi})
 *
 * k is fixed to 1.
 */
...
double V_e1_scf(struct background *pba,
  ...
  double lambda1 = pba->scf_parameters[0];
  return background_safe_exp(-lambda1*phi);
}
...
double dV_e1_scf(struct background *pba,
  ...
  double lambda1 = pba->scf_parameters[0];
  return -lambda1*background_safe_exp(-lambda1*phi);
}
...
double ddV_e1_scf(struct background *pba,
  ...
  double lambda1 = pba->scf_parameters[0];
  return lambda1*lambda1*background_safe_exp(-lambda1*phi);
}

double V_e2_scf(struct background *pba,
                double phi
                ) {
  double lambda2 = pba->scf_parameters[1];
  double A       = pba->scf_parameters[2];
  return A*background_safe_exp(-lambda2*phi);
}
...
double dV_e2_scf(struct background *pba,
  ...
  double lambda2 = pba->scf_parameters[1];
  double A       = pba->scf_parameters[2];
  return -A*lambda2*background_safe_exp(-lambda2*phi);
}
...
double ddV_e2_scf(struct background *pba,
  ...
  double lambda2 = pba->scf_parameters[1];
  double A       = pba->scf_parameters[2];
  return A*lambda2*lambda2*background_safe_exp(-lambda2*phi);
}

double lambda_eff_scf(struct background *pba,
                      double phi
                      ) {
  ...
  double e1 = background_safe_exp(-lambda1*phi);
  double e2 = background_safe_exp(-lambda2*phi);

  double denom = e1 + A*e2;

  if (fabs(denom) < 1e-300) {
    return lambda1;
  }

  return (lambda1*e1 + A*lambda2*e2)/denom;
}

double V_scf(struct background *pba,
             double phi
             ) {
  double V1 = pba->scf_parameters[3];
  return V1 * (V_e1_scf(pba,phi) + V_e2_scf(pba,phi));
}
...
double dV_scf(struct background *pba,
  ...
  double V1 = pba->scf_parameters[3];
  return V1 * (dV_e1_scf(pba,phi) + dV_e2_scf(pba,phi));
}
...
double ddV_scf(struct background *pba,
  ...
  double V1 = pba->scf_parameters[3];
  return V1 * (ddV_e1_scf(pba,phi) + ddV_e2_scf(pba,phi));
}
